...
def write_figures(summary: pd.DataFrame, output_dir: Path) -> None:
    ...
    ax.set_xlabel("Shift strength $\\lambda$")
    ax.set_ylabel("Coverage / theoretical lower bound")
    ax.set_ylim(0.0, 1.02)
    ax.legend()
    fig.tight_layout()
    fig.savefig(output_dir / "coverage_bound_diagnostics.pdf")
    fig.savefig(output_dir / "coverage_bound_diagnostics.png", dpi=200)
    plt.close(fig)
